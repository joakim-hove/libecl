...
static void ecl_grid_volume_test__(const double *x, const double *y, const double *z, double expected) {
  ecl_cell_type cell;
  for (int i=0; i < 8; i++) {
    cell.corner_list[i].x = x[i];
    cell.corner_list[i].y = y[i];
    cell.corner_list[i].z = z[i];
  }
  cell.cell_flags = 0;
  double v1 = ecl_cell_get_volume(&cell);
  double abs_diff = fabs(v1 - expected);
  double rel_diff = abs_diff / expected;
  printf("Libecl tetrahedron: %20.12f \n", v1);
  printf("expected value ...: %20.12f \n", expected);
  printf("abs_diff:%14.12f  rel_diff:%g\n\n", abs_diff, rel_diff);
}

void ecl_grid_volume_test() {
  const double x1[8] = {488100.140035,488196.664549,488085.584866,488182.365605,488099.065709,488195.880889,488084.559409,488181.633495};
  const double y1[8] = {6692539.945578,6692550.834909,6692638.574346,6692650.086244,6692538.810649,6692550.080826,6692637.628127,6692649.429649};
  const double z1[8] = {2841.856000,2840.138000,2842.042000,2839.816000,2846.142000,2844.252000,2846.244000,2843.868000};
  double exp1 = 40368.7852157;

  const double x2[8] = {489539.050892,489638.562319,489527.025803,489627.914272,489537.173839,489638.562319,489524.119410,489627.914272};
  const double y2[8] = {6695907.426615,6695923.399538,6696003.688945,6696020.073168,6695908.526577,6695923.399538,6696005.533276,6696020.073168};
  const double z2[8] = {2652.859000,2651.765000,2652.381000,2652.608000,2655.276000,2651.765000,2656.381000,2652.608000};
  double exp2 = 15766.9187847524;

  const double x3[8] = {486819.009056,486904.877179,486812.975440,486900.527416,486818.450563,486903.978383,486812.975440,486900.527416};
  const double y3[8] = {6694966.253697,6694998.360834,6695061.104896,6695086.717018,6694967.135567,6695000.197284,6695061.104896,6695086.717018};
  const double z3[8] = {2795.193000,2799.997000,2792.240000,2793.972000,2795.671000,2800.927000,2792.240000,2793.972000};
  double exp3 = 3268.8819007839;

  const double x4[8] = {489194.711544,489259.232449,489206.220219,489294.099099,489194.711544,489254.725623,489201.723535,489289.423088};
  const double y4[8] = {6694510.504054,6694525.862296,6694608.410134,6694621.029382,6694510.504054,6694526.272988,6694608.819829,6694621.467114};
  const double z4[8] = {2740.7340,2754.7810,2730.0150,2726.1080,2740.7340,2758.3530,2733.9490,2730.1080};
  double exp4 = 23391.491723456;

  ecl_grid_volume_test__(x1,y1,z1,exp1);
  ecl_grid_volume_test__(x2,y2,z2,exp2);
  ecl_grid_volume_test__(x3,y3,z3,exp3);
  ecl_grid_volume_test__(x4,y4,z4,exp4);
}
